...
OPERATION_NAME_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
RESERVED_EXTERNAL_PREFIXES = ("core.",)
...
BUILTIN_MODULES: dict[str, FeatureModule] = {
    ...
    "advanced.ai": FeatureModule(
        id="advanced.ai",
        description="Optional AI profiles, frame scoring, clip judging, review learning, and missed-moment discovery",
        category="advanced",
    ),
...
def run_module_diagnostics(cwd: str | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if module and module.diagnostics:
            try:
                checks.append(_normalize_module_diagnostics(row["id"], module.diagnostics()))
            except Exception as exc:  # pragma: no cover - defensive external module boundary
                checks.append(
                    {
                        "module": row["id"],
                        "checks": [
            ...
            checks.append(_module_dependency_check(row["id"]))
    return {
        "generated": datetime.now().isoformat(),
        "config": module_config_path(cwd),
        "modules": rows,
        "checks": checks,
        "external_errors": discover_external_module_errors(),
    }
...
def example_operation(context, params):
    ...
    return {{
        "output": output,
        "status": "ok",
        "artifact_kind": "community_example",
        "schema_version": "videoedit.community_example.v1",
    }}


def diagnose():
    return {{
        "module": "community.{package}",
        "checks": [
            {{
                "name": "example_dependency",
                "type": "optional",
                "available": True,
                "message": "Replace with real dependency checks.",
            }}
        ],
    }}
...
def get_module():
    return {{
        ...
        "presets": {{
            "{package}_example": {{
                "name": "{package}_example",
                "description": "Example community preset",
                "requires_modules": ["community.{package}"],
                "steps": [
                    {{
                        "name": "example",
                        "operation": "{package}_example",
                        "params": {{"output": "${{output}}/community_example.json"}},
                    }}
                ],
            }}
        }},
...
def test_module_metadata():
    ...
    assert module["presets"]["{package}_example"]["requires_modules"] == ["community.{package}"]
    assert module["diagnostics"]()["checks"][0]["available"] is True
''',
    )
    _write_file(
        files["readme"],
...
videoedit init {package}_example --output {package}_example.yaml
```
...
- Use a stable dotted module ID such as `community.{package}`.
- Keep imports lightweight. Optional providers should be checked in diagnostics, not imported eagerly.
- Operations should read and write JSON-first artifacts with a `schema_version` and `artifact_kind`.
- Presets should declare `requires_modules` for every optional module they need.
- Do not override built-in operation names. Use a package prefix for operation names.
- Include tests for module metadata, diagnostics, operation output, and preset registration.
""",
    )
    return files
...
def discover_external_modules() -> dict[str, FeatureModule]:
    return _discover_external_modules_with_errors()[0]


def discover_external_module_errors() -> list[dict[str, str]]:
    return _discover_external_modules_with_errors()[1]


def _discover_external_modules_with_errors() -> tuple[dict[str, FeatureModule], list[dict[str, str]]]:
    ...
    for entry_point in entry_points:
        try:
            ...
            _validate_external_module(module)
            ...
            errors.append({"entry_point": entry_point.name, "error": str(exc)})
            continue
...
def _coerce_module(value: Any, source: str) -> FeatureModule:
    ...
    for preset in module.presets.values():
        if isinstance(preset, dict):
            required = [str(item) for item in preset.get("requires_modules", [])]
            if module.id not in required:
                required.append(module.id)
            preset["requires_modules"] = required
    return module


def _validate_external_module(module: FeatureModule) -> None:
    if not MODULE_ID_RE.match(module.id):
        raise ValueError(f"invalid module id: {module.id}")
    if module.id in BUILTIN_MODULES:
        raise ValueError(f"module id conflicts with built-in module: {module.id}")
    if module.id.startswith(RESERVED_EXTERNAL_PREFIXES):
        raise ValueError(f"module id uses reserved prefix: {module.id}")
    if module.core:
        raise ValueError("external modules cannot declare core=True")
    if not module.description:
        raise ValueError(f"module {module.id} requires a description")
    for operation in module.operations:
        if not OPERATION_NAME_RE.match(operation.name):
            raise ValueError(f"invalid operation name: {operation.name}")
        if operation.name in OPERATION_MODULES:
            raise ValueError(f"operation conflicts with built-in operation: {operation.name}")
        if not callable(operation.func):
            raise ValueError(f"operation {operation.name} func is not callable")
    for preset_name, preset in module.presets.items():
        if not OPERATION_NAME_RE.match(str(preset_name)):
            raise ValueError(f"invalid preset name: {preset_name}")
        if not isinstance(preset, dict):
            raise ValueError(f"preset {preset_name} must be a mapping")
        if "steps" not in preset:
            raise ValueError(f"preset {preset_name} requires steps")
...
def _normalize_module_diagnostics(module_id: str, value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ValueError("module diagnostics must return a mapping")
    checks = value.get("checks", [])
    if not isinstance(checks, list):
        raise ValueError("module diagnostics checks must be a list")
    normalized = []
    for index, check in enumerate(checks, 1):
        if not isinstance(check, dict):
            raise ValueError(f"diagnostic check {index} must be a mapping")
        normalized.append(
            {
                "name": str(check.get("name") or f"check_{index}"),
                "type": str(check.get("type") or "module"),
                "available": bool(check.get("available", False)),
                "path": check.get("path"),
                "message": check.get("message"),
            }
        )
    return {"module": str(value.get("module") or module_id), "checks": normalized}
